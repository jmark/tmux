...
=item B<C<client {client-spec}>>

=item B<C<session {session-spec}>>

=item B<C<window {window-spec}>>

=item B<C<pane {pane-spec}>>
...
B<NOTE> that this will not affect the original tmux commands:
